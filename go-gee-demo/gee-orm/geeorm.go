...
// Transaction execute all operation in f TxFunc in one transaction
func (engine *Engine) Transaction(f TxFunc) (result interface{}, err error) {
	// put all operation into transaction function f
	s := engine.NewSession()
...
func (engine *Engine) Migrate(value interface{}) error {
	_, err := engine.Transaction(func(s *session.Session) (result interface{}, err error) {
		...
		// remove columns - create tmp table and rename it
		if len(delCols) == 0 {
			return
		}
